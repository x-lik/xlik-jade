...
func (app *App) Pickup() {
	...
	if fileutil.IsDir(tempDir) == false {
		// 没有则构建新的temp
		CopyPath(w3xDir+"/map", tempDir+"/map")
		pterm.Debug.Println("构建临时区[w3x(map)->map]")
		CopyPath(w3xDir+"/table", tempDir+"/table")
	...
	// 物编ini判定
	if GetModTime(w3xDir+"/table") > GetModTime(tempDir+"/table") {
	...
	CopyPath("embeds/lni/resource", tempDir+"/resource")
	pterm.Info.Println("覆盖同步[lni(resource)->resource]")
	if fileutil.IsDir(w3xDir + "/resource") {
		CopyPath(w3xDir+"/resource", tempDir+"/resource")
		pterm.Info.Println("额外资源[w3x(resource)->resource]")
	}
	// 小地图判定
	if GetModTime(w3xDir+"/war3mapMap.blp") > GetModTime(tempDir+"/resource/war3mapMap.blp") {
